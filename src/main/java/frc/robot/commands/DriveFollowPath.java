...
public class DriveFollowPath extends CommandBase {
    ...
    @Override
    public void initialize() {
        RobotContainer.m_drivetrainSubsystem.useAprilTags(false);
        RobotContainer.m_drivetrainSubsystem.enableBrakeMode(true);
        timer.reset();
        timer.start();
        Pose2d initialPose = trajectory.getInitialPose();
        if(resetOdometry) RobotContainer.m_drivetrainSubsystem.resetOdometry(new Pose2d(initialPose.getTranslation(), RobotContainer.m_drivetrainSubsystem.getAngleRotation2d()));
    }
    ...
    public void execute(){
        ...
        RobotContainer.m_drivetrainSubsystem.drive(targetSpeeds);
    }

    @Override
    public void end(boolean interrupted) {
        timer.stop();
        RobotContainer.m_drivetrainSubsystem.useAprilTags(true);
        RobotContainer.m_drivetrainSubsystem.drive(0, 0, 0, true);
    }
